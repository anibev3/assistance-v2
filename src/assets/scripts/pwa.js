...
$(document).ready(function(){      
        ...
        function showInstallPromotion(){
            if(!$('body').hasClass('is-installed')){
                if($('#menu-install-pwa-android, .add-to-home').length){
                    ...
                    console.log('La div #menu-install-pwa-android n\'a pas été trouvée. Veuillez ajouter cette div pour afficher la fenêtre d\'installation')
                }
            }
        }
        let deferredPrompt;
    ...
    if (isMobile.iOS()) {
        if(!isInWebAppiOS){
            ...
            if($('#menu-install-pwa-ios, .add-to-home').length){
                ...
                console.log('La div #menu-install-pwa-ios n\'a pas été trouvée. Veuillez ajouter cette div pour afficher la fenêtre d\'installation')
            }
        }
    }    
    ...
    function check_version(){
        if($('link[data-pwa-version]').length){
            function versionCheck(){        
                ...
                var onlineVersionNumber = "Connexion hors ligne. En attente de reconnexion";
                ...
                setTimeout(function(){
                    ...
                    if(onlineVersionNumber != localVersionNumber && onlineVersionNumber != "Connexion hors ligne. En attente de reconnexion"){
                        updateModal();
                        console.log('Nouvelle version de contenu disponible. Mise à jour à effectuer. Sur les navigateurs de bureau, une actualisation manuelle peut être nécessaire.')
                        setTimeout(function(){
    ...
    if(!offlineAlerts.length){
        $('body').append('<p class="offline-message bg-red2-dark color-white center-text uppercase ultrabold">Aucune connexion internet détectée</p> ');
        $('body').append('<p class="online-message bg-green1-dark color-white center-text uppercase ultrabold">Vous êtes de retour en ligne</p>');
    }
